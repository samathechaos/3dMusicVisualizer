...
void setup() {
  ...
  jingle = minim.loadFile("frogCroak.mp3", 1024); //load song file
  ...
  void render() {
    ...
    for (int y = 1; y < waves.size(); y++) {
      ...
      for(int x=1; x < w1.sz; x++) {
        fill( 10, 255-w1.bandMap[x]  * 9,120);
...
//Sama Manchanda
//code attribute to processing user theinhibitor for certain methods

...
int numS = 100;        //Const that dictates how many bands will get read
...
void setup() {
  ...
  jingle = minim.loadFile("Mountain-stream.mp3", 4096); //load song file
...
void draw()
{
  ...
  background(0);  
  pointLight(200, 102, 255, 65, 200, 100);        //Some 3D lightning for neat effect
  pointLight(200, 40, 60, -65, -60, -150);  
  ambientLight(70, 70, 70);                     // Raise overall light in scene 
  ...
  void render() {
    ...
    for (int y = 1; y < waves.size(); y++) {
      ...
      for(int x=1; x < w1.sz; x++) {
        fill(w1.bandMap[x]  * 9, w1.bandMap[x]  * 12, 255);
...
//Sama Manchanda
// attributes to processing user: theinhibitor
